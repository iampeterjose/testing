...
const ProfileNav = ({ toggleNav, isOpen }) => {
    ...
    return (
        <>
            <div
                className={`fixed inset-0 z-[100] flex md:hidden transition-all ${isOpen ? "pointer-events-auto" : "pointer-events-none"}`}
                style={{ minHeight: "100dvh", maxHeight: "100dvh" }}
            >
                {/* Overlay */}
                <div
                    className={`absolute inset-0 bg-black/40 transition-opacity z-40 ${isOpen ? "block pointer-events-auto" : "hidden pointer-events-none"}`}
                    onClick={toggleNav}
                />
                ...
                >
                    ...
                    <nav className="mt-16 flex flex-col items-center flex-1 overflow-y-auto">
                        <div className="flex flex-col items-center mb-6">
                            ...
                            <p className="text-base font-semibold text-orange-800">{session?.user?.email || "Guest"}</p>
                        </div>
                        <ul className="w-full flex flex-col gap-2">
                            ...
                            {session?.user ? (
                                ...
                                <li>
                                    <Link href="/api/auth/signin" onClick={handleNavigation} className="block w-full px-6 py-3 rounded-xl text-md font-semibold bg-orange-600 text-white hover:bg-orange-700 transition text-center">
                                        Sign In
                                    </Link>
                                </li>
                            )}
